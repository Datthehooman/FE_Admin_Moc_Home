...
const validateForm = () => {
  ...
  if (voucherForm.discount_value === null || isNaN(voucherForm.discount_value) || voucherForm.discount_value <= 0)
  ...
  if (voucherForm.start_date && voucherForm.end_date && new Date(voucherForm.start_date) > new Date(voucherForm.end_date))
    errors.end_date = 'Ngày kết thúc phải sau ngày bắt đầu.';
  if (voucherForm.usage_limit !== null && (isNaN(voucherForm.usage_limit) || voucherForm.usage_limit <= 0))
    errors.usage_limit = 'Số lượt sử dụng phải lớn hơn 0.';
  if (voucherForm.min_order_value !== null && (isNaN(voucherForm.min_order_value) || voucherForm.min_order_value < 0))
    errors.min_order_value = 'Giá trị đơn hàng tối thiểu không hợp lệ.';
  if (voucherForm.max_discount_amount !== null && (isNaN(voucherForm.max_discount_amount) || voucherForm.max_discount_amount < 0))
    errors.max_discount_amount = 'Giá trị giảm tối đa không hợp lệ.';
...
const submitForm = async () => {
  ...
  try {
    ...
    alert('Thêm voucher thất bại: ' + (err.response ? JSON.stringify(err.response.data) : err.message));
  } finally {
    loading.value = false;
  }
};
