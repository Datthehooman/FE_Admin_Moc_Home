...
async function deleteProduct(event) {
    confirm.require({
        ...
        message: `Bạn có chắc muốn xóa sản phẩm "${product.value.product_name}" không?`,
...
<template>
    <div class="p-6 bg-white rounded shadow" v-if="!loading && product">
        <!-- Nút Quay lại -->
        <div class="mb-4">
            <Button label="Quay lại" icon="pi pi-arrow-left" severity="secondary" text @click="router.push('/Product/ProductList')" />
        </div>
        ...
        <div class="flex flex-col lg:flex-row gap-8">
            ...
            <div class="lg:w-1/2 flex flex-col gap-4">
                ...
                <div class="mt-6 flex gap-2">
                    ...
                    <Button label="Xóa sản phẩm" icon="pi pi-trash" severity="danger" text @click="deleteProduct($event)" />
                </div>
            </div>
        </div>
        <ConfirmPopup />
