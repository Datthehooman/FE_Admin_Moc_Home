  ...
  <template v-if="conv.last_message">
    <template v-if="getMessageImages(conv.last_message).length">
      📷 Hình ảnh
    </template>
    <template v-else>
  ...
  <template v-else>
    ...
    if (msg.image_url && typeof msg.image_url === 'string') {
        try { msg.image_url = JSON.parse(msg.image_url); } catch {}
    }
    return msg;
    }
  ...
  try {
    const res = await apiClient.get('/conversation', { headers: { Authorization: `Bearer ${authStore.token}` } });
    conversations.value = res.data.result.data.conversations || [];

    // tự động chọn conversation đầu tiên nếu chưa có chọn
    ...
    // --- Poll messages tự động ---
    async function pollMessages() {
    if (!selectedConversation.value) return;

    try {
        // Lấy page cuối cùng (mới nhất)
        const resFirst = await apiClient.get(`/conversation/${selectedConversation.value.id}/message?page=1`, {
        headers: { Authorization: `Bearer ${authStore.token}` }
        });
        const lastPage = resFirst.data.result.data.messages.last_page;

        const resLast = await apiClient.get(`/conversation/${selectedConversation.value.id}/message?page=${lastPage}`, {
        headers: { Authorization: `Bearer ${authStore.token}` }
        });

        const fetchedMessages = resLast.data.result.data.messages.data || [];
    ...
    try {
        ...
        newMsgs.forEach(msg => {
            // kiểm tra có tempMsg trùng id không, update tempMsg
            const index = messages.value.findIndex(m => m.isTemp && m.id === tempId);
            if (index !== -1) {
